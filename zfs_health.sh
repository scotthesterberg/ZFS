...
if [ -f ~/variables.txt ]; then
	...
	MAIL="Variables definition file missing! As a result ZFS health could not be run"
	printf "$MAIL" | mail -s "ZFS health failed!" $email
	exit 1
fi
...
if [ "$problems" -ne 0 ]; then
 echo -e "$emailSubject \n\n\n `/sbin/zpool list` \n\n\n `/sbin/zpool status`" | mail -s "$emailSubject" $email
 logger $emailSubject
fi

#if [ "$problems" -eq 0 ]; then
 #this is helpful if running command manually, but otherwise generates unesseary mail from cron
 #echo "ZFS Healthy"
#fi
